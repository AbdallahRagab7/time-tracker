...
export function ProjectGrid({ projects, onEdit, onDelete }: ProjectGridProps) {
  ...
  const getProjectStats = (projectId: string) => {
    ...
    const billableTime = projectEntries.filter((e) => e.billable).reduce((sum, e) => sum + e.duration, 0)
          ...
          <Card key={project.id} className="border-border bg-card hover:border-primary/50 transition-colors">
            <CardHeader>
              <div className="flex items-start justify-between">
                ...
                <div
                  className={`text-xs px-2 py-1 rounded ${project.status === "active" ? "bg-primary/20 text-primary" : "bg-muted text-muted-foreground"}`}
                >
                  {project.status}
                </div>
              </div>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex gap-2 pt-4 border-t border-border">
                <Button onClick={() => onEdit(project)} variant="outline" size="sm" className="flex-1 gap-2">
