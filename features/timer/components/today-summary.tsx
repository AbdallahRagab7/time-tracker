    ...
    <Card className="border-border bg-card">
      ...
      <CardContent className="space-y-4">
        <div className="space-y-2">
          ...
          <div className="text-3xl font-bold text-primary">{formatTime(stats.totalTime)}</div>
        </div>

        <div className="space-y-2">
          <div className="text-sm text-muted-foreground">Billable</div>
          <div className="text-2xl font-bold text-foreground">{formatTime(stats.billableTime)}</div>
        </div>

        <div className="space-y-2">
          <div className="text-sm text-muted-foreground">Entries</div>
          <div className="text-2xl font-bold text-foreground">{stats.entryCount}</div>
        </div>

        {Object.keys(stats.byProject).length > 0 && (
          <div className="space-y-3 border-t border-border pt-4">
            <div className="text-sm font-semibold text-foreground">By Project</div>
            {Object.entries(stats.byProject).map(([projectId, duration]) => {
              const project = projects.find((p) => p.id === projectId)
              return (
                <div key={projectId} className="flex justify-between items-center text-sm">
